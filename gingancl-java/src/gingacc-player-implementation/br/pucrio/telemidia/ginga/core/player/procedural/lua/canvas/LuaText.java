...
public class LuaText extends LuaColoredDraw{
	private String text;
	private Font font;
	private Rectangle bounds;
	//private Graphics graphics;
	
	public LuaText(LuaCanvas parentCanvas,int x, int y, Font font, Color color, String text){
		super(parentCanvas,x,y,color);
		this.font = font;
		if(text == null)
			text="";
		else
	...
	public void draw(Graphics g) {
		...
		g.setFont(font);
		bounds = g.getFontMetrics().getStringBounds(text, 0, text.length(), g).getBounds();
		Point point = this.getPosition();
		//Point parentCanvasPositon = this.getParentCanvasPosition();
		g.drawString(text, 
				point.x /*+ parentCanvasPositon.x*/, 
				point.y /*+ parentCanvasPositon.y*/);
		g.setFont(oldFont);
		g.setColor(oldColor);
	}
