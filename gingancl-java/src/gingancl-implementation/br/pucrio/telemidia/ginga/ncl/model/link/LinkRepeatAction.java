...
public class LinkRepeatAction extends LinkSimpleAction implements
		ILinkRepeatAction {
	private long repetitions;

	private double repetitionInterval;

	public LinkRepeatAction(IFormatterEvent event, short actionType) {
		super(event, actionType);
		this.repetitions = 0;
	...
	public void setRepetitionInterval(double delay) {
		this.repetitionInterval = delay;
	}

	public void run() {
		if (event instanceof IPresentationEvent) {
			((IPresentationEvent)event).setRepetitionSettings(repetitions,
					repetitionInterval);
		}
		super.run();
	}
