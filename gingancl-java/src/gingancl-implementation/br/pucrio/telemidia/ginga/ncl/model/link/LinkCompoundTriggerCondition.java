...
public class LinkCompoundTriggerCondition extends LinkTriggerCondition
		implements ILinkCompoundTriggerCondition {
	...
	public void addCondition(ILinkCondition condition) {
		...
		if (condition instanceof ILinkTriggerCondition) {
			((ILinkTriggerCondition)condition).setTriggerListener(this);
		}
	}
	...
	public List getEvents() {
		List events;
		int i, size;
		ILinkCondition condition;

		events = new Vector();
		size = conditions.size();
		for (i = 0; i < size; i++) {
			condition = (ILinkCondition)conditions.get(i);
			events.addAll(condition.getEvents());
		}
		return events;
	}
	...
	public void evaluationStarted() {
		super.notifyConditionObservers(ILinkTriggerListener.EVALUATION_STARTED);
	}
