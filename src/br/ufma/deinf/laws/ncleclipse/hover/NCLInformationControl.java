...
public class NCLInformationControl extends AbstractInformationControl implements
	...
	protected void createContent(Composite parent) {
		...
		pageConnector = new Canvas(this.internalComposite, SWT.BACKGROUND
	...
	private void loadPreviewConnector() {
		...
		ConnY = 50 * (preViewConnector.getActionRole().size() > preViewConnector
				.getConditionRole().size() ? preViewConnector.getActionRole()
				.size() : preViewConnector.getConditionRole().size());
		ConnY += 10;

		final Vector<Attributes> conditionRole = preViewConnector
				.getConditionRole();
		final Vector<Attributes> actionRole = preViewConnector.getActionRole();
		...
		for (int i = 0; i < conditions.length; i++)
			conditions[i] = new Image(pageConnector.getDisplay(), path + "cond"
					+ conditionRole.get(i).getAttribute("role") + ".png");
		for (int i = 0; i < actions.length; i++)
			actions[i] = new Image(pageConnector.getDisplay(), path + "action"
					+ actionRole.get(i).getAttribute("role") + ".png");
		final String compCondition = preViewConnector.getCompoundCondition();
		...
		if (!compAction.equals("")) {
			compoundAction = new Image(pageConnector.getDisplay(), path + "op"
					+ compAction.toUpperCase() + ".png");
		}
		...
		if (img1 == null) {
			...
			for (int i = 0; i < conditions.length; i++) {
				...
				gc.drawImage(img, 0, 0, img.getBounds().width,
						img.getBounds().height, desX, i * desY + meio,
				...
				if (!max.equals("") && !max.equals("1")) {
					if (max.equals("unbounded"))
						max = "n";
					toShow += max;
					gc.drawText(toShow, desX + DEFAULT_WIDTH, i * desY + meio,
							true);
			...
			if (!compCondition.equals("")) {
				...
				gc.drawImage(tempCondition, 0, 0,
						tempCondition.getBounds().width, tempCondition
								.getBounds().height, desX, meio, DEFAULT_WIDTH,
						DEFAULT_HEIGHT);
			...
			if (!compAction.equals("")) {
				gc.drawImage(tempAction, 0, 0, tempAction.getBounds().width,
						tempAction.getBounds().height, desX, meio,
						DEFAULT_WIDTH, DEFAULT_HEIGHT);
				gc.drawLine(x1, y1, desX, meio + (int) DEFAULT_HEIGHT / 2);
				x1 = desX + DEFAULT_WIDTH;
				y1 = meio + DEFAULT_HEIGHT / 2;
		...
		hBar.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				...
				pageConnector.scroll(destX, 0, 0, 0, rect.width, rect.height,
						false);
				origin.x = -hSelection;
			}
		});
		...
		vBar.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				...
				Rectangle rect = image.getBounds();
				pageConnector.scroll(0, destY, 0, 0, rect.width, rect.height,
						false);
				origin.y = -vSelection;
			}
		});
