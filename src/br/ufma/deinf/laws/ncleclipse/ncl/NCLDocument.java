...
public class NCLDocument {
	...
	public void addElement(NCLElement element, String id) {
		// TODO Auto-generated method stub
		if (alias != null && !alias.equals("")) {
			element.setAttributeValue("id", alias + "#"
	...
	public Collection getElementsFromPerspective(String tagname,
			String perspective) {
	...
	public Collection getElementsFromCompletePerspective(String tagname,
			String completePerspective) {
		Collection elementsFromPerspective = new ArrayList();
		Collection elements = getElements().get(tagname);
		if (elements != null) {
			Iterator it = elements.iterator();
			while (it.hasNext()) {
				NCLElement nclElement = (NCLElement) it.next();
				if (nclElement.getCompletePerspective().equals(
						completePerspective)) {
					elementsFromPerspective.add(nclElement);
				}
			}
	...
	public Vector<NCLElement> getElements(String tag, String attribute) {
		...
		while (it.hasNext()) {
			...
			while (it2.hasNext()) {
				...
				if (idElement == null
						&& (tag.equals("media") || tag.equals("port") || tag
								.equals("area"))) {
